...
public class ParameterListListener extends AbstractPascalListener {
   ...
   public void enterParameterList(pascalParser.ParameterListContext ctx) {
      if (null != ctx.actualParameter()) {
         for (final ActualParameterContext actualParameterContext : ctx.actualParameter()) {
            final ActualParameterListener actualParameterListener = new ActualParameterListener(getExecutionContext());
            actualParameterListener.enterActualParameter(actualParameterContext);
            values.add(actualParameterListener.getValue());
         }
      }
   }
